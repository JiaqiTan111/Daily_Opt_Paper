...
def _markdown_paper(index: int, item: AnalyzedPaper) -> str:
    ...
    scope = "正文节选解读" if item.provenance.reading_scope.value == "full_text" else "摘要级解读"
    evidence = "\n".join(
...
研究设定：{analysis.setting}

研究动机：{analysis.motivation}

核心洞察：{analysis.insight}

技术挑战：
{challenges}

问题分析：{analysis.analysis}
