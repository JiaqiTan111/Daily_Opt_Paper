...
def _phrase_matches(text: str, terms: Iterable[str]) -> tuple[str, ...]:
    ...
    return tuple(
        ...
        for term in terms
        if re.search(
            r"(?<![a-z0-9])" + re.escape(normalize_text(term)) + r"(?![a-z0-9])", normalized
        )
    )
...
def score_paper(
    ...
    *,
    ...
    # No personal corpus: redistribute its unavailable weight proportionally.
    if not documents:
        available_weight = (
            ranking.topic_weight + ranking.recency_weight + ranking.exploration_weight
        )
        base_score = base_score / available_weight if available_weight else 0.0
...
def rank_papers(
    ...
    *,
    ...
    # Method words alone (e.g. neural network) do not establish OR relevance.
    # An empty relevant pool stays empty instead of filling the LLM budget with noise.
    eligible = [
        item
        for item in scored
        if _phrase_matches(f"{item[0].title} {item[0].abstract}", profile.core_terms)
        and (item[1].base_score >= config.min_prefilter_score or item[1].matched_terms)
    ]
    remaining = sorted(
        eligible,
