...
每天北京时间 08:00 主触发，10:17 和 13:43 检查并恢复未完成的投递。GitHub Actions 可能排队，08:00 是触发时间，非保证送达时间。默认回溯 3 天；源记录为空时自动扩大到 7 天，以覆盖发布延迟。最多分析 60 篇、发布 40 篇、读取正文节选 10 篇。质量不足时不补足数量。
...
首次空日报后若同日恢复获得论文，会补发一次标明修订的有内容日报；重复运行不会反复发送。
