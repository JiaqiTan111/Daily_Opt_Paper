...
def test_offline_pipeline_persists_and_reuses_cache(tmp_path: Path) -> None:
    ...
    assert "研究设定" in (tmp_path / "reports/2026-08-29.md").read_text(encoding="utf-8")
    assert (tmp_path / "site/index.html").is_file()
    assert (tmp_path / "site/archive/2026-08-29.html").is_file()
    assert (tmp_path / "site/daily/2026-08-29/index.html").is_file()
    assert "2026-08-29" in (tmp_path / "site/daily/index.html").read_text(encoding="utf-8")
    assert (tmp_path / "site/library/index.html").is_file()
    assert (tmp_path / "site/topics/index.html").is_file()
    assert (tmp_path / "site/topics/scheduling/index.html").is_file()
    assert (tmp_path / "site/tags/index.html").is_file()
    assert (tmp_path / "site/tags/reinforcement-learning/index.html").is_file()
    ...
    assert "papers/arxiv/2608.12345/" in (tmp_path / "site/feed.xml").read_text(encoding="utf-8")
